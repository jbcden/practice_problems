...
def postorder_traversal(root, cache=[])
  return [] unless root

  if root.left
    postorder_traversal(root.left, cache)
  end

  if root.right
    postorder_traversal(root.right, cache)
  end
...
p postorder_traversal(TreeNode.from_list([1, nil, 2, 3]))
